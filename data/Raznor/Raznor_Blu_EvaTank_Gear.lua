-- \todo Conditionally include EvaTank lua instead of base lua
-- include(player.name..'_'..player.main_job..'_gear.lua') -- Required Gear file.

function user_setup()
	-- Options: Override default values
    state.OffenseMode:options('Normal') -- Normal here is really EvasionTank
...
function init_gear_sets()
    ...
    -- \todo - what weapon?  Sakpata's Sword for 10 DT?  Colada for up to 5 enmity?
    sets.enmity = {
	  ammo = "Sapience Orb",      --  2        "Impatiens",
      head = gear.nyame_head,   -- Rabid Visor has 6, but terrible defensive stats
	  neck = "Loricate Torque",  --  \todo +1 and upgrade for a bunch of extra defense
	  ear1 = "Cryptic Earring",   --  4, and 40 hp -- or Odnowa's +1 for more HP, less enmity
	  ear2 = "Eabani Earring", -- 0, but eva/meva,HP -- friomisi has 2 enmity, nothing else
	  body = gear.nyame_body,        -- 0
	  hands = gear.nyame_hands,      -- 0
	  ring1 = "Provocare Ring",      --  5 -- might prefer defending here for defense
	  ring2 = "Vengeful Ring",         -- 3 -- also has eva, petrov has 4 but no good defense
	  -- back = gear.eva_jse_back,    -- 10  , also 20 agi and 45 eva/20 meva
